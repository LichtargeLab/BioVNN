...
class Prospective(Dependency):
    ...
    def output_pred_mean(self):
        ...
        for cell_line in self.pred[data_type]:
            ...
            df.loc[set(self.nonessential_genes) & set(df.index), 'nonessential_genes'] = 1
            df.loc[set(self.pan_dependent_genes) & set(df.index), 'pan_dependent_genes'] = 1
            df.loc[set(self.essential_genes) & set(df.index), 'essential_genes'] = 1
...
def main():
    ...
    run_suffix = 'prospective' + params['load_result_dir_name']
    ...
    log_dir = os.path.join(result_dir, 'Prospective', start_time)
    set_logging('Prospective', log_dir)

    pa = Prospective(params['analysis_source'], data_dir, result_dir, start_time, params, depmap_ver)
    pa.perform(model_name, params)
